...
if (! $vendors_curr_i_down) {
	$vendors_curr_i_down = array ();
        ...
        $result = $db->query ( "SELECT vc.currency_id, vc.rate, name,vendor_id FROM `" . $tabl_prefix . "_vendors_currency` vc LEFT JOIN `" . $tabl_prefix . "_currency` c ON (c.id = vc.currency_id) WHERE `vendor_id` IN (SELECT `id` FROM `" . $tabl_prefix . "_vendors` ORDER BY `id` ASC) ORDER BY `currency_id` ASC" );
